...
class DockingVina(object):
    ...
    def docking(self, receptor_file, ligand_mol_file, ligand_pdbqt_file, docking_pdbqt_file):
        ...
        run_line = '%s --receptor %s --ligand %s --out %s' % (self.vina_program,
                                                              receptor_file, ligand_pdbqt_file,
                                                              docking_pdbqt_file)
    ...
    def predict(self, mol):
        ...
        receptor_file = self.receptor_file
        ligand_mol_file = '%s/ligand_%s.mol' % (self.temp_dir, 0)
        ligand_pdbqt_file = '%s/ligand_%s.pdbqt' % (self.temp_dir, 0)
        docking_pdbqt_file = '%s/dock_%s.pdbqt' % (self.temp_dir, 0)
        try:
            self.gen_3d(smi, ligand_mol_file)
        except Exception as e:
            print(e)
...
if __name__ == '__main__':
    ...
    affinity_list = dv.predict_batch(['CC', 'COC', 'C1=CC=CC=C1'])
    print(affinity_list)
